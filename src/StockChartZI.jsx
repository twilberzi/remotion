import {
  AbsoluteFill,
  useCurrentFrame,
  interpolate,
  spring,
  Img,
  staticFile,
} from "remotion";

const FONT_SANS  = "'Helvetica Neue', Helvetica, Arial, sans-serif";
const FONT_MONO  = "'SF Mono', 'Fira Code', 'Courier New', monospace";

// ZoomInfo brand colors
const ZI_ORANGE  = "#FF6B35";
const ZI_TEAL    = "#00C4B4";
const ZI_NAVY    = "#1A3D5C";
const ZI_BLUE    = "#4A9EFF";
const ZI_WHITE   = "#FFFFFF";
const ZI_DIM     = "rgba(255,255,255,0.5)";
const ZI_GRID    = "rgba(255,255,255,0.08)";

function fi(frame, start, end, from, to) {
  return interpolate(frame, [start, end], [from, to], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
}

// ─── Full daily data (Jan 2 – Mar 6, 2026) ───────────────────────────────────
const RAW = [
  ["Jan 2",  253.62, 382.23, 9.61],
  ["Jan 6",  262.90, 396.00, 10.49],
  ["Jan 7",  266.12, 398.22, 10.48],
  ["Jan 8",  260.53, 369.90, 10.58],
  ["Jan 9",  259.94, 371.56, 10.40],
  ["Jan 12", 259.40, 375.96, 10.47],
  ["Jan 13", 241.06, 358.89, 10.06],
  ["Jan 14", 239.57, 345.30, 10.08],
  ["Jan 15", 233.53, 329.24, 10.00],
  ["Jan 16", 227.11, 311.88,  9.04],
  ["Jan 20", 220.07, 304.79,  8.79],
  ["Jan 21", 221.58, 302.32,  8.76],
  ["Jan 22", 228.09, 319.50,  9.12],
  ["Jan 23", 228.05, 322.78,  8.82],
  ["Jan 26", 229.40, 328.99,  8.91],
  ["Jan 27", 228.53, 319.66,  8.72],
  ["Jan 28", 227.96, 317.11,  8.72],
  ["Jan 29", 214.08, 281.53,  8.19],
  ["Jan 30", 212.29, 280.00,  8.05],
  ["Feb 2",  210.81, 274.06,  7.81],
  ["Feb 3",  196.38, 245.16,  6.92],
  ["Feb 4",  199.44, 243.93,  6.87],
  ["Feb 5",  189.97, 223.49,  6.78],
  ["Feb 6",  191.35, 232.58,  7.31],
  ["Feb 9",  194.03, 231.08,  7.32],
  ["Feb 10", 193.45, 231.95,  6.63],
  ["Feb 11", 185.00, 209.33,  6.56],
  ["Feb 12", 185.43, 228.95,  6.30],
  ["Feb 13", 189.72, 243.85,  6.36],
  ["Feb 17", 184.29, 247.31,  6.49],
  ["Feb 18", 187.79, 250.14,  6.60],
  ["Feb 19", 185.29, 239.37,  6.43],
  ["Feb 20", 185.16, 233.50,  6.45],
  ["Feb 23", 178.16, 217.36,  6.03],
  ["Feb 24", 185.42, 232.60,  5.99],
  ["Feb 25", 191.75, 245.70,  6.13],
  ["Feb 26", 199.47, 268.52,  6.40],
  ["Feb 27", 194.79, 264.51,  6.21],
  ["Mar 2",  192.95, 263.60,  6.18],
  ["Mar 3",  196.05, 275.37,  6.23],
  ["Mar 4",  193.08, 278.59,  6.24],
  ["Mar 5",  201.39, 291.47,  6.59],
  ["Mar 6",  202.11, 296.56,  6.56],
];

const N      = RAW.length;
const LABELS = RAW.map(r => r[0]);
const CRM    = RAW.map(r => r[1]);
const HUBS   = RAW.map(r => r[2]);
const GTM    = RAW.map(r => r[3]);

// ─── Chart geometry ───────────────────────────────────────────────────────────
const L = 210;
const R = 1700;
const T = 230;
const B = 870;
const W = R - L;
const H = B - T;

const Y_MIN = 150, Y_MAX = 430;
const G_MIN = 4.5, G_MAX = 12;

function xAt(i) { return L + (i / (N - 1)) * W; }
function yL(v)  { return B - ((v - Y_MIN) / (Y_MAX - Y_MIN)) * H; }
function yR(v)  { return B - ((v - G_MIN) / (G_MAX - G_MIN)) * H; }

const LEFT_TICKS  = [160, 200, 240, 280, 320, 360, 400];
const RIGHT_TICKS = [5, 6, 7, 8, 9, 10, 11];
const X_LABELS    = ["Jan 2", "Jan 9", "Jan 16", "Jan 23", "Jan 30", "Feb 6", "Feb 13", "Feb 20", "Feb 27", "Mar 6"];

// ─── Path builders ────────────────────────────────────────────────────────────
function buildLinePath(prices, yFn) {
  let d = "";
  for (let i = 0; i < N; i++) {
    const x = xAt(i);
    const y = yFn(prices[i]);
    if (i === 0) d += `M ${x} ${y}`;
    else {
      const px = xAt(i - 1), py = yFn(prices[i - 1]);
      const cpx = (px + x) / 2;
      d += ` C ${cpx} ${py} ${cpx} ${y} ${x} ${y}`;
    }
  }
  return d;
}

function buildAreaPath(prices, yFn) {
  const line = buildLinePath(prices, yFn);
  return `${line} L ${xAt(N-1)} ${B} L ${xAt(0)} ${B} Z`;
}

function dotAt(prices, yFn, progress) {
  const pos = progress * (N - 1);
  const i0  = Math.floor(pos);
  const i1  = Math.min(N - 1, i0 + 1);
  const t   = pos - i0;
  const x   = xAt(i0) + (xAt(i1) - xAt(i0)) * t;
  const y   = yFn(prices[i0]) + (yFn(prices[i1]) - yFn(prices[i0])) * t;
  return { x, y };
}

// ─── Clip rect ────────────────────────────────────────────────────────────────
function ClipRect({ id, progress }) {
  const clipW = Math.max(0, progress * W + 20);
  return (
    <defs>
      <clipPath id={id}>
        <rect x={L} y={0} width={clipW} height={1080}/>
      </clipPath>
    </defs>
  );
}

// ─── Data label badge (ZI style) ──────────────────────────────────────────────
function DataBadge({ text, x, y, color, frame, delay }) {
  const sp  = spring({ frame: frame - delay, fps: 30, config: { damping: 16, stiffness: 200 } });
  const op  = fi(frame, delay, delay + 8, 0, 1);
  const sc  = 0.8 + sp * 0.2;
  const W2  = text.length * 8 + 28;
  return (
    <g opacity={op}
      transform={`translate(${x}, ${y}) scale(${sc})`}
      style={{ transformOrigin: `${x}px ${y}px` }}>
      {/* Pill background */}
      <rect x={-W2/2} y={-14} width={W2} height={28} rx={14}
        fill={color} opacity={0.15}/>
      {/* Pill border */}
      <rect x={-W2/2} y={-14} width={W2} height={28} rx={14}
        fill="none" stroke={color} strokeWidth={1.5} opacity={0.8}/>
      <text x={0} y={5} fontSize={14} fontWeight={700}
        fontFamily={FONT_SANS} fill={color} textAnchor="middle"
        letterSpacing="0.5">{text}</text>
    </g>
  );
}

// ─── Glow line (ZI style — bright stroke with outer glow) ────────────────────
function GlowLine({ d, color, strokeWidth = 2.5, opacity = 1, dashArray }) {
  return (
    <g opacity={opacity}>
      {/* Outer glow */}
      <path d={d} fill="none" stroke={color} strokeWidth={strokeWidth + 8}
        opacity={0.12} strokeLinecap="round" strokeLinejoin="round"
        strokeDasharray={dashArray}/>
      {/* Inner glow */}
      <path d={d} fill="none" stroke={color} strokeWidth={strokeWidth + 3}
        opacity={0.25} strokeLinecap="round" strokeLinejoin="round"
        strokeDasharray={dashArray}/>
      {/* Core line */}
      <path d={d} fill="none" stroke={color} strokeWidth={strokeWidth}
        strokeLinecap="round" strokeLinejoin="round"
        strokeDasharray={dashArray}/>
    </g>
  );
}

// ─── Timing ───────────────────────────────────────────────────────────────────
// 0–16:    bg + axes fade in
// 16–110:  CRM draws
// 90–180:  HUBS draws
// 155–220: GTM draws
// 220+:    labels + % callouts
// 240–390: hold

export const StockChartZI = () => {
  const frame = useCurrentFrame();

  const bgOp   = fi(frame, 0, 20, 0, 1);
  const crmP   = fi(frame, 16, 110, 0, 1);
  const hubsP  = fi(frame, 90, 180, 0, 1);
  const gtmP   = fi(frame, 155, 220, 0, 1);

  const crmDot  = dotAt(CRM,  yL, crmP);
  const hubsDot = dotAt(HUBS, yL, hubsP);
  const gtmDot  = dotAt(GTM,  yR, gtmP);

  const crmArea  = buildAreaPath(CRM,  yL);
  const crmLine  = buildLinePath(CRM,  yL);
  const hubsArea = buildAreaPath(HUBS, yL);
  const hubsLine = buildLinePath(HUBS, yL);
  const gtmLine  = buildLinePath(GTM,  yR);

  const badgeDelay = 222;

  return (
    <AbsoluteFill style={{ background: "#030813", fontFamily: FONT_SANS, overflow: "hidden" }}>

      {/* ── Background image ── */}
      <Img
        src={staticFile("explained/zoominfo-bg.png")}
        style={{
          position: "absolute", inset: 0,
          width: "100%", height: "100%",
          objectFit: "cover",
          opacity: 0.75,
        }}
      />

      {/* ── Dark overlay to deepen + improve contrast ── */}
      <div style={{
        position: "absolute", inset: 0,
        background: "linear-gradient(160deg, rgba(3,8,19,0.72) 0%, rgba(3,8,19,0.55) 50%, rgba(3,8,19,0.80) 100%)",
      }}/>

      {/* ── Orange top accent bar ── */}
      <div style={{
        position: "absolute", top: 0, left: 0, right: 0,
        height: 4,
        background: `linear-gradient(90deg, ${ZI_ORANGE}, #ff9a35, ${ZI_ORANGE})`,
        opacity: fi(frame, 2, 14, 0, 1),
      }}/>

      {/* ── Title block ── */}
      <div style={{
        position: "absolute", top: 52, left: L,
        opacity: fi(frame, 2, 18, 0, 1),
      }}>
        <div style={{
          fontSize: 13, fontWeight: 700, letterSpacing: "3px",
          color: ZI_ORANGE, textTransform: "uppercase",
          marginBottom: 10,
        }}>
          GTM Intelligence
        </div>
        <div style={{
          fontSize: 38, fontWeight: 700, color: ZI_WHITE,
          letterSpacing: "-0.5px", lineHeight: 1.1,
        }}>
          GTM Platform Stock Performance
        </div>
        <div style={{
          width: 440, height: 1, background: "rgba(255,255,255,0.15)",
          marginTop: 10, marginBottom: 8,
        }}/>
        <div style={{
          fontSize: 14, color: ZI_DIM, fontWeight: 400,
        }}>
          Source: stockanalysis.com · Jan 2 – Mar 6, 2026
        </div>
      </div>

      {/* ── Main SVG chart ── */}
      <svg style={{ position: "absolute", inset: 0 }}
        viewBox="0 0 1920 1080" width="1920" height="1080"
        opacity={bgOp}
      >
        {/* ── Grid ── */}
        {X_LABELS.map((lbl, i) => {
          const xi = RAW.findIndex(r => r[0] === lbl);
          if (xi < 0) return null;
          const x = xAt(xi);
          return (
            <g key={i}>
              <line x1={x} y1={T} x2={x} y2={B}
                stroke={ZI_GRID} strokeWidth={1}/>
              <text x={x} y={B + 32} fontSize={13} fill="rgba(255,255,255,0.35)"
                textAnchor="middle" fontFamily={FONT_SANS}>{lbl}</text>
            </g>
          );
        })}

        {LEFT_TICKS.map((t, i) => {
          const y = yL(t);
          return (
            <g key={i}>
              <line x1={L} y1={y} x2={R} y2={y}
                stroke={ZI_GRID} strokeWidth={1}/>
              <text x={L - 14} y={y + 5} fontSize={13} fill="rgba(255,255,255,0.35)"
                textAnchor="end" fontFamily={FONT_SANS}>${t}</text>
            </g>
          );
        })}

        {RIGHT_TICKS.map((t, i) => (
          <text key={i} x={R + 14} y={yR(t) + 5} fontSize={13}
            fill="rgba(0,196,180,0.5)"
            textAnchor="start" fontFamily={FONT_SANS}>${t}</text>
        ))}

        {/* Axes */}
        <line x1={L} y1={T} x2={L} y2={B}
          stroke="rgba(255,255,255,0.2)" strokeWidth={1}/>
        <line x1={L} y1={B} x2={R} y2={B}
          stroke="rgba(255,255,255,0.2)" strokeWidth={1}/>
        <line x1={R} y1={T} x2={R} y2={B}
          stroke="rgba(0,196,180,0.2)" strokeWidth={1}/>

        {/* Axis labels */}
        <text x={L - 100} y={(T + B) / 2} fontSize={12} fill="rgba(255,255,255,0.3)"
          textAnchor="middle" fontFamily={FONT_SANS}
          transform={`rotate(-90, ${L - 100}, ${(T + B) / 2})`}>
          CRM &amp; HUBS — USD
        </text>
        <text x={R + 100} y={(T + B) / 2} fontSize={12} fill="rgba(0,196,180,0.4)"
          textAnchor="middle" fontFamily={FONT_SANS}
          transform={`rotate(90, ${R + 100}, ${(T + B) / 2})`}>
          GTM — USD
        </text>

        {/* ── Gradient defs for area fills ── */}
        <defs>
          <linearGradient id="crmGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={ZI_ORANGE} stopOpacity={0.35}/>
            <stop offset="100%" stopColor={ZI_ORANGE} stopOpacity={0}/>
          </linearGradient>
          <linearGradient id="hubsGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={ZI_BLUE} stopOpacity={0.25}/>
            <stop offset="100%" stopColor={ZI_BLUE} stopOpacity={0}/>
          </linearGradient>
        </defs>

        {/* ── CRM — orange glow line + gradient fill ── */}
        <ClipRect id="crmClip" progress={crmP} />
        <g clipPath="url(#crmClip)">
          <path d={crmArea} fill="url(#crmGrad)"/>
          <GlowLine d={crmLine} color={ZI_ORANGE} strokeWidth={2.5}/>
        </g>
        {crmP > 0.01 && crmP < 0.999 && (
          <g>
            <circle cx={crmDot.x} cy={crmDot.y} r={10}
              fill={ZI_ORANGE} opacity={0.2}/>
            <circle cx={crmDot.x} cy={crmDot.y} r={5}
              fill={ZI_ORANGE} stroke="white" strokeWidth={1.5}/>
          </g>
        )}

        {/* ── HUBS — blue glow dashed line + gradient fill ── */}
        <ClipRect id="hubsClip" progress={hubsP} />
        <g clipPath="url(#hubsClip)">
          <path d={hubsArea} fill="url(#hubsGrad)"/>
          <GlowLine d={hubsLine} color={ZI_BLUE} strokeWidth={2.5} dashArray="12 5"/>
        </g>
        {hubsP > 0.01 && hubsP < 0.999 && (
          <g>
            <circle cx={hubsDot.x} cy={hubsDot.y} r={10}
              fill={ZI_BLUE} opacity={0.2}/>
            <circle cx={hubsDot.x} cy={hubsDot.y} r={5}
              fill={ZI_BLUE} stroke="white" strokeWidth={1.5}/>
          </g>
        )}

        {/* ── GTM — teal glow line ── */}
        <ClipRect id="gtmClip" progress={gtmP} />
        <g clipPath="url(#gtmClip)">
          <GlowLine d={gtmLine} color={ZI_TEAL} strokeWidth={2.5}/>
        </g>
        {gtmP > 0.01 && gtmP < 0.999 && (
          <g>
            <circle cx={gtmDot.x} cy={gtmDot.y} r={10}
              fill={ZI_TEAL} opacity={0.2}/>
            <circle cx={gtmDot.x} cy={gtmDot.y} r={5}
              fill={ZI_TEAL} stroke="white" strokeWidth={1.5}/>
          </g>
        )}

        {/* ── Inline series badges ── */}
        <DataBadge text="Salesforce (CRM)"
          x={xAt(10)} y={yL(CRM[10]) - 36}
          color={ZI_ORANGE} frame={frame} delay={badgeDelay}/>
        <DataBadge text="HubSpot (HUBS)"
          x={xAt(14)} y={yL(HUBS[14]) + 40}
          color={ZI_BLUE} frame={frame} delay={badgeDelay + 12}/>
        <DataBadge text="ZoomInfo (GTM)"
          x={xAt(6)} y={yR(GTM[6]) - 36}
          color={ZI_TEAL} frame={frame} delay={badgeDelay + 24}/>

        {/* ── % change callouts at right edge ── */}
        {fi(frame, badgeDelay, badgeDelay + 12, 0, 1) > 0.05 && (
          <>
            <g opacity={fi(frame, badgeDelay, badgeDelay + 14, 0, 1)}>
              <line x1={R + 2} y1={yL(CRM[N-1])} x2={R + 36} y2={yL(CRM[N-1])}
                stroke={ZI_ORANGE} strokeWidth={1} strokeDasharray="4 3" opacity={0.6}/>
              <text x={R + 40} y={yL(CRM[N-1]) + 5} fontSize={14} fill={ZI_ORANGE}
                fontFamily={FONT_SANS} fontWeight={700}>-20%</text>
            </g>
            <g opacity={fi(frame, badgeDelay + 12, badgeDelay + 26, 0, 1)}>
              <line x1={R + 2} y1={yL(HUBS[N-1])} x2={R + 36} y2={yL(HUBS[N-1])}
                stroke={ZI_BLUE} strokeWidth={1} strokeDasharray="4 3" opacity={0.6}/>
              <text x={R + 40} y={yL(HUBS[N-1]) + 5} fontSize={14} fill={ZI_BLUE}
                fontFamily={FONT_SANS} fontWeight={700}>-22%</text>
            </g>
            <g opacity={fi(frame, badgeDelay + 24, badgeDelay + 38, 0, 1)}>
              <line x1={R + 2} y1={yR(GTM[N-1])} x2={R + 36} y2={yR(GTM[N-1])}
                stroke={ZI_TEAL} strokeWidth={1} strokeDasharray="4 3" opacity={0.6}/>
              <text x={R + 40} y={yR(GTM[N-1]) + 5} fontSize={14} fill={ZI_TEAL}
                fontFamily={FONT_SANS} fontWeight={700}>-32%</text>
            </g>
          </>
        )}
      </svg>

      {/* ── Legend ── */}
      <div style={{
        position: "absolute", top: 156, right: 228,
        display: "flex", gap: 24, alignItems: "center",
        opacity: fi(frame, 14, 28, 0, 1),
      }}>
        {[
          { color: ZI_ORANGE, dash: false, label: "Salesforce (CRM)"  },
          { color: ZI_BLUE,   dash: true,  label: "HubSpot (HUBS)"    },
          { color: ZI_TEAL,   dash: false, label: "ZoomInfo (GTM) →"  },
        ].map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <svg width={28} height={10}>
              {/* Glow */}
              <line x1={0} y1={5} x2={28} y2={5}
                stroke={s.color} strokeWidth={6} opacity={0.2}
                strokeDasharray={s.dash ? "7 4" : "none"}/>
              {/* Core */}
              <line x1={0} y1={5} x2={28} y2={5}
                stroke={s.color} strokeWidth={2.5}
                strokeDasharray={s.dash ? "7 4" : "none"}/>
            </svg>
            <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontFamily: FONT_SANS }}>
              {s.label}
            </span>
          </div>
        ))}
      </div>

      {/* ── ZoomInfo wordmark area (bottom right) ── */}
      <div style={{
        position: "absolute", bottom: 28, right: 52,
        opacity: fi(frame, 20, 34, 0, 1),
        display: "flex", alignItems: "center", gap: 8,
      }}>
        <div style={{
          width: 8, height: 8, borderRadius: "50%",
          background: ZI_ORANGE,
          boxShadow: `0 0 8px ${ZI_ORANGE}`,
        }}/>
        <span style={{
          fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.5)",
          fontFamily: FONT_SANS, letterSpacing: "1px",
          textTransform: "uppercase",
        }}>ZoomInfo</span>
      </div>

      {/* ── Source line ── */}
      <div style={{
        position: "absolute", bottom: 28, left: L,
        fontSize: 12, color: "rgba(255,255,255,0.25)", fontFamily: FONT_SANS,
        opacity: fi(frame, 18, 32, 0, 1),
      }}>
        Data: stockanalysis.com · All prices in USD · Past performance is not indicative of future results
      </div>

    </AbsoluteFill>
  );
};
